...
aggpath<-"E:/LocalNaip/2_aggregated"
...
#batches$Location[curbatches]<-"Castilleja"
...
alldone<-list.files(aggpaths.done)
...
for(i in 2:length(rawfiles)){
  ...
  for(i in 1:length(indpaths)){setwd(indpaths[i]);fl<-list.files();file.remove(fl)}
  setwd(localcpath)
}
