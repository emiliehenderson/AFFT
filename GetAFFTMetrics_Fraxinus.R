library(terra);library(AFFT)
rm(list = ls())
gc()
localpath<-"D:/LocalNaip"

localpath2<-"E:/LocalNaip"
rawpath<-"N:/mpsg_naip"
rawlocalpath<-"E:/LocalNaip/0_raw"
indpath<-paste(localpath,"1_intermediate",sep = "/")
indpath2<-paste(localpath2,"1_intermediate",sep = "/")
ndvipath<-paste(indpath,"ndvi",sep = "/")
ndgrpath<-paste(indpath,"ndgr",sep = "/")
ndngpath<-paste(indpath,"ndng",sep = "/")
bripath<-paste(indpath,"bri",sep = "/")
ndvipath2<-paste(indpath2,"ndvi",sep = "/")
ndgrpath2<-paste(indpath2,"ndgr",sep = "/")
ndngpath2<-paste(indpath2,"ndng",sep = "/")
bripath2<-paste(indpath2,"bri",sep = "/")
aggpath<-paste(localpath,"2_aggregated",sep = "/")
ndvipath_a<-paste(aggpath,"ndvi",sep = "/")
ndgrpath_a<-paste(aggpath,"ndgr",sep = "/")
ndngpath_a<-paste(aggpath,"ndng",sep = "/")
bripath_a<-paste(aggpath,"bri",sep = "/")
rpath_a<-paste(aggpath,"r",sep = "/")
gpath_a<-paste(aggpath,"g",sep = "/")
npath_a<-paste(aggpath,"n",sep = "/")
#batches<-read.csv("N:/mpsg_naip_batch_files/BatchLog_v2.csv")
#print(batches)
#curbatch<-12

#rawfiles<-read.csv(paste("N:/mpsg_naip_batch_files/naip_batch_",curbatch,".txt",sep = ""))[,1]
#rawfiles<-sapply(strsplit(rawfiles,"/"),function(x){x[3]})
rawfiles<-list.files(rawlocalpath)
indexfiles<-list.files(c(ndvipath,ndgrpath,ndngpath,bripath))
indexfiles<-table(indexfiles)
indexfiles<-names(indexfiles[indexfiles>=4])
rfl<-rawfiles<-rawfiles[!rawfiles %in% indexfiles]
rawfiles<-paste(rawpath,rawfiles,sep = "/");names(rawfiles)<-rfl

setwd(localpath2)
localraw<-gsub(rawpath,rawlocalpath,rawfiles)
#file.copy(rawfiles,localraw)
localraw<-localraw[file.info(localraw)$size > 200000000]
rawpath<-rawlocalpath

gc()
library(snowfall)
time2<-system.time({
  sfInit(parallel = T, cpus = 10)
  sfLibrary(AFFT)
  sfLibrary(terra)
  sfExport(list =c("rawpath","aggpath","localpath","indexfiles","localraw"))
  stime<-Sys.time()
  sfExport("stime")
  write("",paste(localpath,"donefiles.txt",sep = "/"),append = F)
  sfLapply(localraw,function(x){
    
    terraOptions(memfrac = .9/10,datatype = "FLT4S")
    GetBandIndices(x,ncpu = 1,outpath = indpath)
    write(x,paste(localpath,"donefiles.txt",sep = "/"),append = T)
    donefiles<-readLines(paste(localpath,"donefiles.txt",sep = "/"))
    donefiles<-donefiles[2:length(donefiles)]
    te<-difftime(Sys.time(), stime,units = "secs")
    prop.done<-length(donefiles)/length(localraw)
    tr<-te*(1-prop.done)/(prop.done)
    hms<-SDMap::SecsToHMS(tr)
    write(paste(round(prop.done * 100,3),"% done"),paste(localpath,"progress.txt",sep = "/"),append = F)
    write("",paste(localpath,"progress.txt",sep = "/"),append = T)
    hms<-SDMap::SecsToHMS(te)
    
    write("Time Elapsed:",paste(localpath,"progress.txt",sep = "/"),append = T)
    write(paste(paste(hms,names(hms)),collapse = "   "),paste(localpath,"progress.txt",sep = "/"),append = T)
    
    hms<-SDMap::SecsToHMS(tr)
    
    write("Time Remaining:",paste(localpath,"progress.txt",sep = "/"),append = T)
    write(paste(paste(hms,names(hms)),collapse = "   "),paste(localpath,"progress.txt",sep = "/"),append = T)
  })
  sfStop()
})


indexfiles<-table(paste(ndvipath,list.files(c(ndvipath,ndgrpath,ndngpath,bripath),full.names = F),sep = "/"))
indexfiles<-names(indexfiles)[indexfiles == 4]
aggfiles<-paste(ndvipath,list.files(c(ndvipath_a,ndgrpath_a,ndngpath_a,bripath_a,rpath_a,gpath_a,npath_a),full.names = F),sep = "/")
aggfiles<-table(aggfiles)
aggfiles<-names(aggfiles)[aggfiles==7]

indexfiles<-indexfiles[!indexfiles %in% aggfiles]
ind<-0
print(length(indexfiles))
indexfiles<-indexfiles[order(file.info(indexfiles)$size,decreasing = F)]
setwd(localpath)
library(snowfall)
time2<-system.time({
  #terraOptions(memfrac = .95)
  sfInit(parallel = T, cpus = 15)
  sfLibrary(AFFT)
  sfLibrary(terra)
  sfExport(list =c("rawpath","aggpath","localpath","indexfiles"))
  stime<-Sys.time()
  sfExport("stime")
  write("",paste(localpath,"donefiles.txt",sep = "/"),append = F)
  
  sfClusterApplyLB(indexfiles,function(x){
    
    terraOptions(datatype = "FLT4S")
    GetAFFT(x,rawpath = rawpath,
            indpath = paste(localpath,"1_intermediate",sep = "/"),
            aggpath = paste(localpath,"2_aggregated",sep = "/"),
            ncpu = 1,overwrite = T)
    write(x,paste(localpath,"donefiles.txt",sep = "/"),append = T)
    donefiles<-readLines(paste(localpath,"donefiles.txt",sep = "/"))
    donefiles<-donefiles[2:length(donefiles)]
    te<-difftime(Sys.time(), stime,units = "secs")
    prop.done<-length(donefiles)/length(indexfiles)
    tr<-te*(1-prop.done)/(prop.done)
    hms<-SDMap::SecsToHMS(tr)
    write(paste(round(prop.done * 100,3),"% done"),paste(localpath,"progress.txt",sep = "/"),append = F)
    write("",paste(localpath,"progress.txt",sep = "/"),append = T)
    hms<-SDMap::SecsToHMS(te)
    
    write("Time Elapsed:",paste(localpath,"progress.txt",sep = "/"),append = T)
    write(paste(paste(hms,names(hms)),collapse = "   "),paste(localpath,"progress.txt",sep = "/"),append = T)
    
    hms<-SDMap::SecsToHMS(tr)
    
    write("Time Remaining:",paste(localpath,"progress.txt",sep = "/"),append = T)
    write(paste(paste(hms,names(hms)),collapse = "   "),paste(localpath,"progress.txt",sep = "/"),append = T)
  })
  sfStop()
})
# 
# fl<-list.files("2_aggregated/ndvi",full.names = T)
# r1<-rast(fl[3])
# plot(r1,col = rainbow(255))

band<-"ndvi"#readline("Band name:")
cols<-rainbow(255,alpha = ((1:255)+200)/455)

lastind<-length(aggfiles)
aggfiles<-list.files(paste(localpath,"2_aggregated/ndvi",sep ="/"),
                     pattern = ".tif",full.names = F)

junk<-lapply(aggfiles,function(x,p = 2,band = "ndvi"){
  r0<-rast(paste(rawpath,x,sep = "/"))
  r2<-rast(paste(localpath,"1_intermediate",band,x,sep = "/"))
  r3<-rast(paste(localpath,"2_aggregated",band,x,sep = "/"))
  if(p==2){
        par(mfrow =c(2,2))
   # r4<-app(r3,function(x){c(x[1],x[2],sum(x[3:4]))/sum(x[1:4])*255})
    r4<-subset(r3,c(7:9))
  
    e1<-ext(r4)#/5
    plotRGB(r4,ext = e1,smooth = F,stretch = "lin")
    plotRGB(c(r2,r2,r2),stretch = "lin",ext = e1,smooth = F)
    #plotRGB(r4,ext = e1)
    #plotRGB(r4,stretch = "hist",ext = e1)
    plotRGB(c(r3$mean,r3$mean,r3$mean),stretch = "lin",ext = e1,smooth = F)
    plotRGB(r0,ext = e1,smooth = F)#,stretch = 'lin')
  }else if (p==1){
    plot(subset(r3,1:12), col = cols,smooth = F)
  }

  readline(paste(which(aggfiles==x),"out of",length(aggfiles)))
  })




## Things to think about ----------------
### 1) Workflow. Should GetBandIndices be rolled in to GetAFFT?
### 2) Parallel handling. GetBandIndices currently uses 4 cpus (one per index). Could re-jigger the parallelization to send each image out to a separate cpu, but how would memory work?
### 3) File structure -- improvements needed?
### 4) Tracking -- how to keep track of what is and isn't done when working on multiple platforms?


lapply(tmplist,function(x){
  y<-rast(x)
  z<-rast(gsub(ndvipath2,rawpath,x))
  par(mfrow =c(1,2))
  plotRGB(z,stretch = "lin")
  plot(y);readline(x)})

